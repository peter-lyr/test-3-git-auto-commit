...
bool bsp_record_encode_init(void)
{
    rec_cb_t *rec = &rec_cb;
...
#if REC_MP3_SUPPORT
    if (rec->src->rec_type == REC_MP3) {
        if (rec->src->nchannel & BIT(5)) {
            rec->trigger_len = 384 << (rec->src->nchannel & 0x03);      //layer2
        } else {
            rec->trigger_len = 576 << (rec->src->nchannel & 0x03);      //layer3
        }
